...
def does_zip_have_nifti(uploaded_file):
    """Проверяет наличие NIfTI файлов в zip"""
...
def load_nifti_file(file_path):
    """Загружает NIfTI файл и возвращает данные и заголовок"""
    try:
...
def create_3d_surface_plot(data, isovalue=0.5, opacity=0.7):
    """Создает 3D поверхность из объемных данных"""
    try:
...
def create_slice_plots(data, slice_type='axial'):
    """Создает графики срезов"""
    if slice_type == 'axial':
    ...
    fig = px.imshow(slice_data, 
                    ...
                    title=f'{slice_type.capitalize()} Slice')
    return fig

def create_orthogonal_slices(data):
    """Создает ортогональные срезы"""
    fig = make_subplots(
        rows=2, cols=2,
...
st.set_page_config(page_title='Advanced 3D Medical Visualization', layout='wide')

st.title("🧠 Advanced 3D Medical Imaging & EEG Analysis")
st.markdown("### Comprehensive visualization tool for NIfTI files and EEG data")

st.sidebar.title("Navigation")
page = st.sidebar.selectbox("Choose a page", ["Upload & Overview", "3D Visualization", "Slice Analysis", "EEG Analysis", "Statistics"])
...
st.markdown("---")
st.markdown("### 🛠️ Advanced Medical Visualization Tool")
st.markdown("**Features:** NIfTI 3D visualization, slice analysis, EEG processing, statistical analysis")
st.markdown("**Built with:** Streamlit, Plotly, Nibabel, MNE-Python, Scikit-image")

if st.session_state.data_dir and st.button('🗑️ Clear All Data'):
